...
export async function PATCH(req: Request, { params }: { params: Promise<{ account_id: string }> }) {
  try {
    ...
    try {
      ...
      console.log('[api/account PATCH] token present=', !!token, 'token_len=', token?.length ?? 0);
    ...
    if (!upstream.ok) {
      try {
        console.error('[api/account PATCH] upstream returned', upstream.status, 'payload=', payload);
      } catch {
        // noop
      }
    }
