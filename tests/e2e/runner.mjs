...
const tests = fs.existsSync(e2eDir)
  ? fs.readdirSync(e2eDir).filter((f) => f.endsWith('.mjs') && f !== runnerName).map((f) => path.resolve(e2eDir, f))
  : [];
...
function runTest(file) {
  return new Promise((resolve, reject) => {
    ...
    const onFinish = (code, signal) => {
      clearTimeout(timer);
      if (code === 0) resolve();
      else reject(new Error(`${file} exited with code ${code}${signal ? ' signal:' + signal : ''}`));
    };
